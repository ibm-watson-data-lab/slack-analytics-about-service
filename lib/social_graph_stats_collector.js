	...
	this.GraphClient.gremlin({gremlin:'def g = graph.traversal(); g.V().has("isUser", true).has("userName","' + userName + '");'},
		                     function(err, dbResponse) {
		                     	...
		                     	if((dbResponse.result.data) && (dbResponse.result.data.length > 0)) {
		                     		return callback(null, {vertexId: dbResponse.result.data[0].id, userName: dbResponse.result.data[0].properties.userName[0].value, userId: dbResponse.result.data[0].properties.userId[0].value} );
		                     	}
		                     	else {
		                     		return callback({code:404,message:'User ' + userName + ' is unknown.'});
		                     	}		                     	
		                     });
 }; // fetchUserInfo
 					   ...
 					   function(err, stats) {
							...
							var response = {response_type: 'ephemeral',
						   					...
						   					attachments: [
						   								   ...
						   								   {
						   								   	text: 'Most frequently mentioned in channels: ' + statsMap.channel_mentioned_in.top_5,
						   								   	mrkdwn_in: ['text']
						   								   },
						   								   {
						   								   	text: 'Most frequently mentioned channels: ' + statsMap.channel_mentions.top_5,
						   								   	mrkdwn_in: ['text']
						   								   }
						   					             ]
						   				   };
