...
const debug = require('debug')('slack-about-service');
 ...
 KeywordStatsCollector.prototype.fetchKeywordStats = function (keyword, keywordMatches, responseUrl) {
 		...
 		if((! keyword) || (! keywordMatches) || (keywordMatches.length === 0) || (! responseUrl)) {
 			...
 			console.error('		responseUrl   : ' + responseUrl);
 					   ...
 					   function(err, results) {
							...
							const response = {response_type: 'ephemeral',
						   						text: 'Statistics for keyword *' + keyword + '*\n' +
						   					    	  // ' Mentioned by: ' + _.flatMap(_.sortBy(_.uniqBy(results.users,'id'),'name'),function(user) {return '<@' + user.id + '|' + user.name + '>';}).join(', ') + '\n' + 
						   					    	  ' Mentioned by: ' + _.flatMap(_.sortBy(_.uniqBy(results.users,'id'),'name'),function(user) {return '@' + user.name;}).join(', ') + '\n' + 
						   					    	  ' Mentioned in: ' + _.flatMap(_.sortBy(_.uniqBy(results.channels,'id'),'name'),function(channel) {return '<#' + channel.id + '|' + channel.name + '>';}).join(', '),
						   						mrkdwn: true
						   				   	 };			   		   	
